...
const OrdersContainer = ({ alertActions, token }) => {
		...
		(async () => {
			...
			switch (tabValue) {
				...
				case 1:
					response = await Service.api.provider.getOrder(
						page + 1,
						`open=${true}&page_size=${rowsPerPage}`,
						token);
					break;
				case 2:
					response = await Service.api.provider.getOrder(
						page + 1,
						`page_size=${rowsPerPage}&shipping_status=NOT_READY`,
						token);
					break;
				case 3:
					response = await Service.api.provider.getOrder(
						page + 1,
						`page_size=${rowsPerPage}&not_payed=${true}`,
						token);
					break;
				default:
					break;
			}
			rowData = response?.body?.collection?.reduce((accum, orders) => {
				accum.push({
					provider_id   : orders.products[0].provider_id,
					order         : orders.code,
					date          : orders.timestamp,
					provider_name : orders.trade_name,
	...
	}, [page, tabValue, rowsPerPage, stFlags, stShipping, order, filter, token]);
